...
{
  const cwd = path.join(raiz, 'angular-doctor');
  if (!existsSync(path.join(cwd, 'node_modules'))) ejecutar('npm install', cwd);
  ejecutar('npx ng build --base-href /angular/', cwd);
}
...
cpSync(path.join(raiz, 'dist'), path.join(salida, 'vue'), { recursive: true });
cpSync(path.join(raiz, 'angular-doctor', 'dist', 'angular-doctor', 'browser'), path.join(salida, 'angular'), { recursive: true });
